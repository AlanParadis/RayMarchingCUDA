...
// Modified version of https://github.com/JAGJ10/Snow/blob/fa154b77c8d75d159aeea5fbf6eee46ff938717b/Snow/matrix.h
// credits :
/**************************************************************************
**
...
struct mat3 {
    ...
    */
	__host__ __device__ __forceinline__ static mat3 rotateX(float theta) {
		float c = cos(theta);
		float s = sin(theta);
		return mat3(
			1, 0, 0,
	 ...
	 */
	__host__ __device__ __forceinline__ static mat3 rotateY(float theta) {
		float c = cos(theta);
		float s = sin(theta);
		return mat3(
			c, 0, s,
	...
	*/
	__host__ __device__ __forceinline__ static mat3 rotateZ(float theta) {
		float c = cos(theta);
		float s = sin(theta);
		return mat3(
			c, -s, 0,
